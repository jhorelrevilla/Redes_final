...
//DB Sql("test1.db");
DB Sql;
string create_json_atributos(string nodo_consulta){
    vector <Atr> consulta=Sql.ReadAtributosNodo(nodo_consulta);
    vector <string> esquema={"id","atributo","valor"};
    string result="[";
    for(auto i:consulta){
        result+="{";
        result=result+esquema[0]+":"+to_string(i.id)+",";
        result=result+esquema[1]+":"+string(i.atrib)+",";
        result=result+esquema[2]+":"+string(i.value)+"},";
    }
